...
class _PickupVerificationPageState extends State<PickupVerificationPage> {
  ...
  bool get _codeVerified => _verification.code;

  /// 当前已经通过的验证项数量。
  int get _verifiedCount => _verification.pickupVerifiedCount;

  /// 四个验证项是否全部通过。
  bool get _allVerified => _verification.allPickupVerified;
  ...
  void _goNextIfAllVerified() {
    ...
    _hasNavigated = true;
    Navigator.of(context).pushReplacementNamed(AppRoutes.verificationLoading);
  }
  ...
  void _appendCode(String value) {
    ...
    final nextCode = _pickupCode + value;
    setState(() {
      _pickupCode = nextCode;
      _verification = _verification.copyWith(code: nextCode.length == 8);
    });
    _goNextIfAllVerified();
  }
  ...
  void _deleteCode() {
    ...
    final nextCode = _pickupCode.substring(0, _pickupCode.length - 1);
    setState(() {
      _pickupCode = nextCode;
      _verification = _verification.copyWith(code: nextCode.length == 8);
    });
  ...
  void _clearCode() {
    ...
    setState(() {
      _pickupCode = '';
      _verification = _verification.copyWith(code: false);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return TerminalShell(
      ...
      child: _PickupFrame(
        ...
        faceVerified: _verification.face,
        fingerprintVerified: _verification.fingerprint,
        nfcVerified: _verification.nfc,
        pickupCode: _pickupCode,
        codeVerified: _codeVerified,
        onFaceVerified: () =>
            _updateVerification(_verification.copyWith(face: true)),
        onFingerprintConfirm: () =>
            _updateVerification(_verification.copyWith(fingerprint: true)),
        onNfcConfirm: () =>
            _updateVerification(_verification.copyWith(nfc: true)),
